...
- **[AGENT_INSTRUCTIONS.md](./AGENT_INSTRUCTIONS.md)** - **START HERE!** Complete guide on how to use Context Engine V2
- **[QUICK_REFERENCE.md](./QUICK_REFERENCE.md)** - One-page cheat sheet for quick lookup
- **[AGENT_PROMPT.md](./AGENT_PROMPT.md)** - Copy-paste snippet for agent system prompts
- **[V2_TEST_RESULTS.md](./V2_TEST_RESULTS.md)** - Test results showing performance, features, and examples

---

## 🎯 For AI Coding Agents: How to Gather Context

This system replaces traditional file reading with intelligent semantic search + multi-hop reasoning, providing **67-77% token savings** compared to reading full files.

### **Quick Reference**

| Traditional Method | Context Engine V2 Method | Token Savings |
|-------------------|----------------------|---------------|
| Read 5+ full files with `view` | 1 intelligent API query | **67-77%** |
| ~9,200 tokens | ~2,988 tokens | **3x more efficient** |
...
# Terminal 2: Start Context Engine API (V2 only)
npm start        # Intelligent multi-hop engine on port 3457
```
...
### **Step 1: Query for Context** (Use this instead of `view` tool)

**Query Pattern (V2 - intelligent multi-hop on :3457):**
```bash
curl -X POST http://localhost:3457/api/context \
  ...
  -d '{
    ...
    "limit": 10,
    "strategy": "auto",       // minimal | auto | architectural
    "maxTokens": 3000          // token budget for layered contexts
  }'
```

**V2 Response Highlights:**
- `layers`: `{ L1, L2, L3 }` → immediate, dependency, and downstream contexts
- `meta`: `{ strategy, maxTokens, totalTokens, layerCounts, rawCounts }`
- `agentView.primary`: string you can drop directly into the system/context prompt
- `agentView.secondary`: short summaries of additional contexts
- `agentView.reference.files`: list of related files for quick navigation
...
- [ ] Verify health: `curl http://localhost:3457/health`
